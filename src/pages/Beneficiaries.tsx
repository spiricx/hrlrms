...
export default function Beneficiaries() {
  ...
  useEffect(() => {
    const fetchBeneficiaries = async () => {
      const {
        data,
        error
      } = await supabase.from('beneficiaries').select('*').order('created_at', {
        ascending: false
      });
      if (!error && data) {
  ...
  const filtered = beneficiaries.filter(b => {
    const matchesSearch = b.name.toLowerCase().includes(search.toLowerCase()) || b.employee_id.toLowerCase().includes(search.toLowerCase());
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        {isAdmin && <Select value={stateFilter} onValueChange={setStateFilter}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">MONTHLY RPMT</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map(b => <tr key={b.id} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <Link to={`/beneficiary/${b.id}`} className="text-sm font-medium text-accent hover:underline">
                      View
                    </Link>
                  </td>
                </tr>)}
