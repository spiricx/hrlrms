...
export default function Reports() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const {
        data
      } = await supabase.from('beneficiaries').select('*');
  ...
  const filtered = useMemo(() => stateFilter === 'all' ? beneficiaries : beneficiaries.filter(b => b.state === stateFilter), [beneficiaries, stateFilter]);
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="bg-card rounded-xl shadow-card p-6">
          ...
          <div className="flex justify-center gap-6 mt-2">
            {statusData.map(s => <div key={s.name} className="flex items-center gap-2 text-xs">
        ...
        <div className="bg-card rounded-xl shadow-card p-6">
          <h2 className="text-lg font-bold font-display mb-4">Loans by Organizations (₦M)</h2>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
