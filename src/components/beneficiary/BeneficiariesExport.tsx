...
interface Props {
  records: BenExportRow[];
  staffName: string;
}

const BANK_NAME = 'FEDERAL MORTGAGE BANK OF NIGERIA';
const TITLE = 'Beneficiaries Report';
const HEADERS = [
  'S/N', 'Beneficiary Name', 'Organization', 'NHF No', 'Loan Ref No', 'State', 'Branch',
  'Tenor', 'Loan Amount (₦)', 'Outstanding (₦)', 'Total Repaid (₦)', 'Monthly Repayment (₦)',
  'Last Pmt Amt (₦)', 'Last Pmt Date', 'Overdue Amt (₦)', 'Mths Overdue',
  'Arrears Amt (₦)', 'Mths Arrears', 'Status',
];
...
function toRow(r: BenExportRow, i: number): (string | number)[] {
  return [
    i + 1, r.name, r.organization, r.nhfNumber, r.loanRefNo, r.state, r.branch,
    formatTenor(r.tenor), r.loanAmount, r.outstanding, r.totalPaid, r.monthlyRepayment,
    r.lastPmtAmt, r.lastPmtDate, r.overdueAmt, r.monthsOverdue,
    r.arrearsAmt, r.monthsArrears, r.status,
  ];
}
...
// Excel
export function exportBeneficiariesExcel(records: BenExportRow[], staffName: string) {
  ...
  ws['!cols'] = HEADERS.map((_, i) => ({ wch: i === 0 ? 6 : i >= 8 ? 18 : 16 }));
  XLSX.utils.book_append_sheet(wb, ws, 'Beneficiaries');
  const buf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
  saveAs(new Blob([buf], { type: 'application/octet-stream' }), `Beneficiaries_Report.xlsx`);
  toast.success('Excel exported');
}

// PDF
export async function exportBeneficiariesPDF(records: BenExportRow[], staffName: string) {
...
// Print
export function printBeneficiaries(records: BenExportRow[], staffName: string) {
  const html = `<html><head><title>${TITLE}</title>
    ...
    ${records.map((r, i) => `<tr>
      <td>${i + 1}</td><td>${r.name}</td><td>${r.organization}</td><td>${r.nhfNumber}</td>
      <td>${r.loanRefNo}</td><td>${r.state}</td><td>${r.branch}</td>
      ...
      <td>${r.lastPmtDate}</td>
      <td class="text-right">${formatCurrency(r.overdueAmt)}</td>
      <td class="text-center">${r.monthsOverdue}</td>
...
export default function BeneficiariesExport({ records, staffName }: Props) {
  return (
    <div className="flex gap-2">
      <Button size="sm" variant="outline" onClick={() => exportBeneficiariesPDF(records, staffName)}>
        <FileText className="w-4 h-4 mr-1" /> PDF
      </Button>
      <Button size="sm" variant="outline" onClick={() => exportBeneficiariesExcel(records, staffName)}>
        <FileSpreadsheet className="w-4 h-4 mr-1" /> Excel
      </Button>
      <Button size="sm" variant="outline" onClick={() => printBeneficiaries(records, staffName)}>
